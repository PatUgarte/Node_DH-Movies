...
${getStringPeliculasConDatos(movies, false)}`);
			break;
		case '/mas-votadas':
			res.end(`MÁS VOTADAS
...
Contáctanos:
¿Tenés algo para contarnos? Nos encanta escuchar a nuestros clientes. Si deseas contactarnos podés escribirnos al siguiente email:	dhmovies@digitalhouse.com	o en nuestras redes sociales @dhmoviesOK. 

Envianos tu consulta, sugerencia o reclamo y será respondido a la brevedad. Recordá que también podes consultar la sección de Preguntas Frecuentes para obtener respuestas inmediatas a los problemas más comunes.
`);
			break;
...
function getStringPeliculasOrdenadasAlfabeticamente(arrayPeliculas) {
	let peliculasOrdenadasAlfabeticamente = arrayPeliculas.sort((a, b) => a.title > b.title ? 1 : -1);
...
function getPeliculasMasVotadas(arrayPeliculas) {
	return arrayPeliculas.filter(({ vote_average }) => vote_average >= 7 ? true : false);
}
